...
class Retention{
  ...
  deleteObject(typeName, rc){
    return new Promise((resolve, reject) => {
      console.log(`deleting ${typeName} for ${rc.name}...`);
      let labels = rc.labels;
      this.k8s[typeName].deleteBySelector({app: labels.app, version: labels.version}, (err, deploys) => {
        if (err){
          if (!err.statusCode || err.statusCode != 404){
            return reject(err);
          }
    ...
    return new Promise((resolve, reject) => {
      async.eachLimit(expired, this.agentOptions.parallelism || 5, (rc, cb1) => {
          this.deleteService(rc)
            ...
            .then(rc => this.deleteObject('deployments', rc))
